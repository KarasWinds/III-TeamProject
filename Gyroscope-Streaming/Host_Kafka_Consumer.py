...
def statusvote(df, clf):
    ...
    if statusscale[3] > 0.4:
        print("狀態 ==> {st}, 比例 ==> {sc}".format(st=status[3], sc=str(statusscale[3] * 100) + "%"))
    else:
...
if __name__ == "__main__":
    ...
    svdf = CatchDataFrame(svdfcol_name, 10)
    ...
    try:
        ...
        for record in consumer:
            ...
            if stddf.pointer == stddf.num:
                # print(stddf)
                stddf.pointer_zero()
            if svdf.pointer == stddf.num:
                # print(svdf)
                svdf.pointer_zero()
                # print(svdf.ix[:, :len(Data)])
            ...
            print(svdf.iloc[svdf.pointer, :].tolist())
            # print(svdf)
            svdf.pointer_add()
    except:
        # 錯誤處理
