    ...
    def downloadaudio_all(
        ...
        for link in self.video_urls:
            try:
                ...
                # TODO: this should not be hardcoded to a single user's
                # preference
                dl_stream = yt.streams.filter(adaptive=True, only_audio=True).order_by("abr").desc().first()

                logger.debug('download path: %s', download_path)
                if prefix_number:
                    prefix = next(prefix_gen)
                    logger.debug('file prefix is: %s', prefix)
                    dl_stream.download(download_path, filename_prefix=None)
                else:
                    dl_stream.download(download_path)
                logger.debug('download complete')
